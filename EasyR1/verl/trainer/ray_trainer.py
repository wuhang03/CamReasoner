...
class RayPPOTrainer:
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # repeat to align with repeated responses in rollout
            print(f"Repeating new batch with n={self.config.worker.rollout.n}...")
            new_batch = new_batch.repeat(repeat_times=self.config.worker.rollout.n, interleave=True)
            new_batch = new_batch.union(gen_batch_output)

            # filter group
            print("Filtering generated samples...")
            if self.config.algorithm.online_filtering:
                ...
                # Debug: print all avg_score values
                print("Online filtering avg_scores:")
                for uid, avg_score in uid2mean.items():
                    print(f"  uid={uid} avg_score={avg_score}")
                print("low and high thresholds:")
                print(f"  low={self.config.algorithm.filter_low}")
                print(f"  high={self.config.algorithm.filter_high}")
                kept_uids = [
                    uid
                    for uid, avg_score in uid2mean.items()
                    if avg_score > self.config.algorithm.filter_low and avg_score < self.config.algorithm.filter_high
                ]
                print(f"Kept {len(kept_uids)} samples out of {len(uid2mean)} after filtering.")
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                if (
                    ...
                    print(f"Validation metrics at step {self.global_step}:\n{convert_dict_to_str(unflatten_dict(val_metrics))}")

                if self.config.trainer.save_freq > 0 and self.global_step % self.config.trainer.save_freq == 0:
                    with timer("save_checkpoint", timing_raw):
                        self._save_checkpoint()
                        print(f"Checkpoint saved to {self.config.trainer.save_checkpoint_path} at step {self.global_step}.")
